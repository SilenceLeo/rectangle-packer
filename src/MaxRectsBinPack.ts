...
export default class MaxRectsBinPack {
    ...
    InsertRects(rects: RectSize[], method: FreeRectChoiceHeuristic): Rect[] {
        ...
        while (rects.length > 0) {
            ...
            for (let i = 0; i < rects.length; ++i) {
                ...
                const newNode: Rect = this.ScoreRect(rects[i].width, rects[i].height, method, score1, score2);
    ...
    FindPositionForNewNodeBottomLeft(width: number, height: number, bestY: RefNumber, bestX: RefNumber): Rect {
    ...
    FindPositionForNewNodeBestShortSideFit(
        ...
        for (let i = 0; i < this.freeRectangles.length; ++i) {
            ...
            if (this.binAllowFlip && this.freeRectangles[i].width >= height && this.freeRectangles[i].height >= width) {
                const flippedLeftoverHoriz = Math.abs(this.freeRectangles[i].width - height);
                const flippedLeftoverVert = Math.abs(this.freeRectangles[i].height - width);
        ...
        for (let i = 0; i < this.freeRectangles.length; ++i) {
            ...
            if (this.freeRectangles[i].width >= width && this.freeRectangles[i].height >= height) {
                ...
                if (
                    ...
                    (areaFit === bestAreaFit.value && shortSideFit < bestShortSideFit.value)
                ) {
                    bestNode.x = this.freeRectangles[i].x;
                    bestNode.y = this.freeRectangles[i].y;
                    bestNode.width = width;
    ...
    FindPositionForNewNodeContactPoint(width: number, height: number, bestContactScore: RefNumber): Rect {
        const bestNode: Rect = new Rect();
    ...
    PruneFreeList(): void {
        ...
        for (let i = 0; i < this.freeRectangles.length; ++i)
            for (let j = i + 1; j < this.freeRectangles.length; ++j) {
                if (Rect.IsContainedIn(this.freeRectangles[i], this.freeRectangles[j])) {
                    this.freeRectangles.splice(i, 1);
                    --i;
                    break;
                }
                if (Rect.IsContainedIn(this.freeRectangles[j], this.freeRectangles[i])) {
